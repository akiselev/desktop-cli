...
fn get_window_info(conn: &RustConnection, window_id: u32) -> Result<WindowInfo> {
    ...
    let pid_prop = conn.get_property(false, window_id, net_wm_pid, AtomEnum::CARDINAL, 0, 1)
        .map_err(|e| crate::error::DesktopCliError::Platform(format!("Failed to get property: {}", e)))?
        .reply()
        .map_err(|e| crate::error::DesktopCliError::Platform(format!("Failed to get property reply: {}", e)))?;
    ...
    let geometry = conn.get_geometry(window_id)
        .map_err(|e| crate::error::DesktopCliError::Platform(format!("Failed to get geometry: {}", e)))?
        .reply()
        .map_err(|e| crate::error::DesktopCliError::Platform(format!("Failed to get geometry reply: {}", e)))?;
