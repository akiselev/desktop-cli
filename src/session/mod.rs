...
impl DesktopSession {
    pub fn new(
        ...
        Self {
            id,
            graph: AccessibilityGraph::new(id),
            accessibility,
            input,
            pack: None,
            event_rx: None,
    ...
    pub fn graph(&self) -> &AccessibilityGraph {
        &self.graph
    }
    pub fn graph_mut(&mut self) -> &mut AccessibilityGraph {
        &mut self.graph
    }
    pub fn backend_kind(&self) -> BackendKind {
        self.accessibility.kind()
    }
    pub fn set_pack(&mut self, pack: Option<ApplicationPack>) {
        self.pack = pack;
        self.graph.clear_aliases();
    }
    pub fn pack(&self) -> Option<&ApplicationPack> {
        self.pack.as_ref()
    }
    pub fn set_limits(&mut self, max_depth: usize, max_children: usize) {
    ...
    pub fn refresh(&mut self) -> Result<u64, String> {
        ...
        for node in existing.into_iter().filter(|n| !seen.contains(n)) {
    ...
    fn sync_subtree(
        ...
        if depth > self.max_depth {
            return Err(format!(
                "accessibility tree exceeded max depth {}",
                self.max_depth
            ));
        }
        ...
        let child_identities = self.accessibility.children(
            native,
            ChildRange {
                offset: 0,
        ...
        if let Some(element) = self.graph.get_mut(node) {
    ...
    pub fn query(&self, selector: &str) -> Result<Vec<ElementRef>, String> {
        ...
        Ok(self
            ...
            .query(&selector)
            .into_iter()
            .map(|node| self.graph.element_ref_for(node))
            .collect())
    }
    ...
    pub fn observe(&mut self, budget: ObservationBudget) -> Result<ProjectionResult, String> {
        if let Some(pack) = self.pack.clone() {
            project(&mut self.graph, &pack, budget)
        } else {
            let observation = observe_raw(&self.graph, budget);
            Ok(ProjectionResult {
                observation,
                active_view: "raw-semantic".into(),
    ...
    pub fn perform(
        ...
        let node = if let Some(alias) = target.strip_prefix('$') {
            self.graph
                .resolve_alias(alias)
                .ok_or_else(|| format!("alias '{}' does not resolve", target))?
        } else if let Some(node) = self.graph.resolve_opaque(target) {
            node
        } else {
            ...
            if matches.len() != 1 {
                return Err(format!(
                    "action target '{}' resolved to {} elements",
                    target,
                    matches.len()
                ));
            }
        ...
        let result = {
            let mut router = ActionRouter {
                graph: &self.graph,
                accessibility: self.accessibility.as_mut(),
                input: self.input.as_deref_mut(),
            };
            router.perform(SemanticActionRequest {
                target: element_ref,
                action,
                value,
                allow_input_fallback,
            })?
        };
        let _ = self.refresh();
        Ok(result)
    ...
    pub fn perform_pack_action(
        ...
        let pack = self
            .pack
            .clone()
            .ok_or_else(|| "no pack is active".to_string())?;
        let action = pack
            .stylesheet
            .actions
            .iter()
            .find(|a| a.name == name)
            .cloned()
            .ok_or_else(|| format!("pack action '{}' does not exist", name))?;
                ...
                if let Some(keys) = action.fallback_keys {
                    {
                        let input = self.input.as_deref_mut().ok_or_else(|| {
                            format!(
                                "{}; pack fallback '{}' requires an input backend",
                                primary, keys
                            )
                        })?;
                        input.keys(&keys)?;
                    ...
                    let node = self
                        .graph
                        .resolve_alias(action.target.trim_start_matches('$'))
                        .or_else(|| self.graph.roots().first().copied())
                        .ok_or_else(|| {
                            "pack action completed but no target reference is available".to_string()
                        })?;
                    Ok(SemanticActionResult {
                        success: true,
                        target: self.graph.element_ref_for(node),
                        action: action.perform,
                        route: "pack-fallback-keys".into(),
                        message: Some(format!(
                            "accessibility route failed: {}; used {}",
                            primary, keys
                        )),
                    })
                } else {
    ...
    pub fn start_events(&mut self) -> Result<(), String> {
        let first_root = self
            ...
            .into_iter()
            .next()
            .ok_or_else(|| "no accessibility root".to_string())?;
        let interests = [
            EventKind::Created,
            EventKind::Destroyed,
        ...
        self.event_rx = Some(
            self.accessibility
                .subscribe(&first_root.native, &interests)?,
        );
        Ok(())
    }
