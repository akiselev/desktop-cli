...
pub struct ActionRouter<'a> {
    ...
    pub input: Option<&'a mut dyn InputBackend>,
}
...
impl<'a> ActionRouter<'a> {
    pub fn perform(&mut self, request: SemanticActionRequest) -> Result<SemanticActionResult, String> {
        ...
        match backend_result {
            Ok(result) if result.success => Ok(SemanticActionResult { success: true, target: request.target, action: request.action, route: "accessibility".into(), message: result.message }),
            Ok(result) if !request.allow_input_fallback => Ok(SemanticActionResult { success: false, target: request.target, action: request.action, route: "accessibility".into(), message: result.message }),
            ...
            backend => {
                ...
                match request.action {
                    ...
                    SemanticAction::SetValue | SemanticAction::SetText | SemanticAction::ReplaceText => {
                        ...
                        if let ActionValue::Text(text) = request.value { input.type_text(&text)?; } else { return Err("text input fallback requires a text value".into()); }
                    }
                ...
                Ok(SemanticActionResult { success: true, target: request.target, action: request.action, route: "input-fallback".into(), message: Some("native accessibility action unavailable; used guarded input fallback".into()) })
            }
        }
    }
}
